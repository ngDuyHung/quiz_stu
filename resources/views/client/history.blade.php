...
<div class="space-y-6 sm:space-y-8 page-fade px-3 sm:px-0">
    ...
    <div class="mb-6 sm:mb-8">
        <h2 class="text-2xl sm:text-4xl font-bold text-primary mb-2">Lịch sử thi</h2>
        <p class="text-xs sm:text-base text-slate-500">Tất cả các lần thi của bạn, sắp xếp mới nhất trước</p>
    </div>
    ...
    @else
        ...
        @endphp
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 sm:gap-6 mb-2">
            <div class="bg-white p-4 sm:p-6 rounded-2xl shadow-sm border border-slate-100 flex items-center gap-3 sm:gap-4">
                <div class="w-10 h-10 sm:w-12 sm:h-12 rounded-xl bg-blue-50 text-blue-600 flex items-center justify-center flex-shrink-0">
                    <span class="material-symbols-outlined text-base sm:text-lg">grading</span>
                </div>
                <div>
                    <p class="text-xs sm:text-sm text-slate-500 font-medium">Tổng lần thi</p>
                    <h3 class="text-xl sm:text-2xl font-bold text-slate-800">{{ $totalCount }}</h3>
                </div>
            </div>
