<nav class="flex flex-col fixed left-0 top-0 h-full w-64 border-r border-slate-200/50 bg-white dark:bg-slate-900 z-50">
    ...
    <div class="flex-1 px-4 space-y-2">
        ...
        <a class="nav-item group flex items-center gap-4 px-4 py-3.5 rounded-2xl transition-all relative overflow-hidden {{ request()->routeIs('client.results') ? 'text-primary' : 'text-slate-600 hover:text-primary' }}" href="{{ route('client.results') }}">
            @if(request()->routeIs('client.results'))
            <div class="absolute inset-0 bg-gradient-to-r from-primary/15 via-primary/10 to-primary/5 rounded-2xl"></div>
            <div class="absolute -right-8 -top-8 w-32 h-32 bg-primary/10 rounded-full blur-2xl"></div>
            @else
            <div class="absolute inset-0 bg-primary/5 rounded-2xl opacity-0 group-hover:opacity-100 transition-opacity"></div>
            @endif
            <span class="material-symbols-outlined relative z-10 @if(request()->routeIs('client.results')) scale-110 @endif group-hover:scale-110 transition-transform" @if(request()->routeIs('client.results')) style="font-variation-settings: 'FILL' 1" @endif>assignment_turned_in</span>
            <span class="text-sm font-medium tracking-wide relative z-10 @if(request()->routeIs('client.results')) font-bold @endif">Kỳ thi đã làm</span>
        </a>
